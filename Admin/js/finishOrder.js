...
async function loadOrders(token) {
    var url = "https://agriculture.cosplane.asia/api/Order/ByStatus?status=FULLPAIDCONFIRMED";
    ...
    if (resultFetch.ok) {
        ...
        pTexts.forEach(element => {
            ...
            <td><button onclick="finishOrder('${element.id}')"  type="button" class="btn btn-success confirmPayment">FINISH</button></td>
        </tr> `
        });
    }
}
function dispatcher(id, name) {
    localStorage.setItem("backAddress", name);
    window.location = "orderDetail.html?id=" + id;
}
async function finishOrder(id) {
    var token = localStorage.getItem("token");
    var url = "https://agriculture.cosplane.asia/api/Order/FinishOrder?orderId=" + id;
