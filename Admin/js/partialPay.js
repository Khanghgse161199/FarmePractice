...
async function loadOrders(token) {
    var url = "https://agriculture.cosplane.asia/api/Order/ByStatus?status=PARTIALPAID";
    ...
    if (resultFetch.ok) {
        ...
        pTexts.forEach(element => {
            ...
            <td class="cusButton"><button onclick="confirmPay('${element.id}')"  type="button" class="btn btn-success confirmPayment">CONFIRM PAYMENT</button><button  onclick="cancelOrder('${element.id}')" type="button" class="btn btn-danger confirmPayment">CANCEL</button></td>
        </tr> `
        });
    }
}
async function confirmPay(id) {
    var token = localStorage.getItem("token");
    console.log(token)
    var url = "https://agriculture.cosplane.asia/api/Order/ConfirmPartialPaid?orderId=" + id;
