...
async function loadOrders(token) {
    var url = "https://agriculture.cosplane.asia/api/Order/ByStatus?status=FULLPAID";
    ...
    if (resultFetch.ok) {
        ...
        pTexts.forEach(element => {
            ...
            <td><button onclick="confirmFullPay('${element.id}')"  type="button" class="btn btn-success confirmPayment">CONFIRM FULLPAID</button></td>
        </tr> `
        });
    }
}
async function confirmFullPay(id) {
    var token = localStorage.getItem("token");
    var url = "https://agriculture.cosplane.asia/api/Order/ConfirmFullPaid?orderId=" + id;
